#mais 3 questoes de cada nivel
medio1 = {'titulo': 'De quem é a famosa frase “Penso, logo existo”?',
'nivel': 'medio',
'opcoes': {'A': 'Platão', 
'B': 'Galileu Galilei',
'C': 'Descartes',
'D': 'Sócrates'},
'correta': 'C' }

medio2 = {'titulo': 'Quais o menor e o maior país do mundo?',
'nivel':'medio' ,
'opcoes': {'A':'Vaticano e Rússia', 'B': 'Nauru e China', 'C':'Mônaco e Canadá' , 'D':'Malta e Estados Unidos' }, 
'correta': 'A'}

medio3 = {'titulo': 'Qual o nome do presidente do Brasil que ficou conhecido como Jango?',
'nivel':'medio' ,
'opcoes': {'A':'Jânio Quadros', 'B': 'Jacinto Anjos', 'C':'João Figueiredo' , 'D':'João Goulart' }, 
'correta': 'D'}


facil1 = {'titulo': 'Qual a montanha mais alta do mundo?',
'nivel':'facil' ,
'opcoes': {'A':' Mauna Kea', 'B': 'Dhaulagiri', 'C':' Monte Chimborazo' , 'D':'Monte Everest' }, 
'correta': 'D'}

facil2 = {'titulo': 'A que temperatura a água ferve?',
'nivel':'facil' ,
'opcoes': {'A':'200 ºC', 'B': '0 ºC', 'C':'180 ºC' , 'D':'100 ºC' }, 
'correta': 'D'}

facil3 = {'titulo': 'Qual o maior planeta do sistema solar?',
'nivel':'facil' ,
'opcoes': {'A':'Marte', 'B': 'Saturno', 'C':'Júpiter' , 'D':'Sol' }, 
'correta': 'C'}


dificil1 = {'titulo': 'Qual o número mínimo de jogadores numa partida de futebol?',
'nivel':'dificil' ,
'opcoes': {'A':'8', 'B': '5', 'C':'9' , 'D':'7' }, 
'correta': 'D'}

dificil2 = {'titulo': ' Quais as duas datas que são comemoradas em novembro?',
'nivel':'dificil' ,
'opcoes': {'A':' Independência do Brasil e Dia da Bandeira', 'B': 'Proclamação da República e Dia Nacional da Consciência Negra', 'C':'Dia do Médico e Dia de São Lucas' , 'D':'Black Friday e Dia da Árvore' }, 
'correta': 'B'}

dificil3 = {'titulo': 'Quanto tempo a luz do Sol demora para chegar à Terra?',
'nivel':'dificil' ,
'opcoes': {'A':'12 minutos', 'B': '8 minutos', 'C':'12 segundos' , 'D':'12 horas' }, 
'correta': 'B'}

dificil4 = {'titulo': 'Em que ordem surgiram os modelos atômicos?',
'nivel':'dificil' ,
'opcoes': {'A': 'Dalton, Thomson, Rutherford, Rutherford-Bohr', 'B':'Dalton, Thomson, Rutherford-Bohr, Rutherford', 'C':'Dalton, Rutherford-Bohr, Thomson, Rutherford' , 'D':'Thomson, Dalton, Rutherford, Rutherford-Bohr' }, 
'correta': 'A'}



#   MODELO DE QUESTAO:

{'titulo': 'oi',
'nivel':'oi' ,
'opcoes': {'A':'oi', 'B': 'oi', 'C':'oi' , 'D':'oi' }, 
'correta': 'oi'}

